...
following sequence: 1. Run
``python compare_rnea_fb_spatial.py INSERT_URDF.urdf -f`` which will
...
and xyz location of Fb joint) 2. After generating compatible inputs with
Matlab, create a floating base URDF.m file by running
``python generate_spatial_model.py INSERT_URDF.urdf -f``. 3. Place the
newly generated ``URDF.m`` file in the working directory of
spatial_v2_extended. 4. Run the script after ensuring spatial
directories are sourced correctly. 5. Suggested testing includes
